...
<<"bcd">>
```
Note that the Size is specified in bits, but the total size of a segment is actually `Size * units`, and for a binary Type the default for units is 8, so the total size of X is `1*8 = 8 bits`.  In case you were wondering, the default units for the other Type's is just 1.
