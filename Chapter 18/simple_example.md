...
Instead, I decided to try [gun](https://github.com/ninenines/gun) for the http client, which has builtin websockets support and is maintained by the same person who maintains cowboy.  gun is for `erlang 18+`, though, so if you need to install a more recent version of erlang, you can use `kerl` or `evm` to install multiple versions of erlang on your system and switch between them as needed.
